...
for i = 2:length(x)-1
    for j = 1:length(t)-1 
        ...
        alpha(i,j) = 2 * (Q(i,j) / A(i,j)) + (((g * A(i,j) / B) - (Q(i,j)^2 / A(i,j)^2)) / ((Q(i,j) / A(i,j)) * (5/3 - (4 * R / (3 * B)))));
        beta = g*A(i)*(Sf - S0);

        Q(i,j+1) = Q(i,j) - (dt/dx)*alpha(i,j)*(Q(i,j)-Q(i-1,j)) + beta*dt;
